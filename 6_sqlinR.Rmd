...
# Track purchase vs. album purchase

The company wants to know how many invoices were for whole albums and how many were for individual tracks. 

To get this information from the tables we have, we will need to use the invoice_line, track and album tables.

If the invoice was for the whole album, then the intersection of the set of tracks associated with an album and the set of tracks associated with an album would be NULL. In queries, this can be written as: 

```{sql, connection=conn, output.var="invoice_album"}
SELECT * 
FROM invoice_line il
INNER JOIN track t on il.track_id = t.track_id
